...
def load_data_and_labels( # Renamed for clarity, was load_data_and_onehot_label
    ...
    noise = torch.randn_like(data_tensor) * no_Power
    data_tensor += noise
...
def Train_data_Gen_with_labels(data_path="./Data/Train_Data.mat", # Renamed for clarity
    ...
    data_tensor = torch.tensor(data_np, dtype=torch.float32)
    label_tensor = torch.tensor(label_np.squeeze(), dtype=torch.long) # Integer labels
    # noise = torch.randn_like(data_tensor) * 5
    # data_tensor += noise
...
class EncoderNetwork(nn.Module):
    def __init__(self, 
        ...
        self.scat = ScatLayer().to(device) # User switched to this version 
    ...
    def forward(self, x):
        ...
        if not hasattr(self, 'attention_module') or \
           ...
           self.attention_module_configured_channels != Z.shape[1]:
            ...
            reduction = current_channels_Z // 8 if current_channels_Z > 128 else 4 # Example logic for reduction
...
def visualize_features_tsne(encoder: EncoderNetwork,
                            ...
                            n_iter_value: int = 1000, # 增加迭代次数以获得更好的收敛
                            limit_samples: Optional[int] = 2000): # 限制用于t-SNE的样本数以提高速度
    """
    使用 t-SNE 可视化编码器输出的特征。

    Args:
        encoder (EncoderNetwork): 训练好的编码器模型。
        dataloader (DataLoader): 提供数据进行可视化的数据加载器。
        device (str): 'cuda' 或 'cpu'。
        num_classes (int): 数据集中的总类别数，用于绘图着色。
        filename (str): 保存 t-SNE 图像的文件名。
        perplexity_value (int): t-SNE 的 perplexity 参数。
        n_iter_value (int): t-SNE 的迭代次数。
        limit_samples (Optional[int]): 用于 t-SNE 的最大样本数, None 表示不限制。
    """
    encoder.eval()
    encoder.to(device)
    
    print(f"\nVisualizing encoder features using t-SNE (perplexity={perplexity_value}, n_iter={n_iter_value})...")
    ...
    with torch.no_grad():
        for inputs, labels_idx in dataloader:
            ...
            if limit_samples is not None and current_samples >= limit_samples:
                print(f"Limiting t-SNE to approximately the first {limit_samples} samples for speed.")
                break
...
if __name__ == "__main__":
    ...
    support_dataset_full, support_classes = Train_data_Gen_with_labels( # Using updated function name
        data_path="./Data/Train_Data_Com.mat", # Path to your primary training data 
        label_path="./Data/Train_Label.mat"
    )
    
    prototype_compute_loader = DataLoader(support_dataset_full, batch_size=proto_eval_batch_size, shuffle=False)
    ...
    **ShuffleNet (2018)**: Combines pointwise group convolution and channel shuffle, significantly reducing computational load while preserving accuracy.
    '''
    model_type = 'efficientnet_v2'  # Example: 'resnet50', 'convnext_v2', 'mobilenet_v3', 'efficientnet_v2',  'shufflenet', 'densenet','resnext',
    ...
    supcon_epochs = 10 # E.g., 20-100 epochs, depends on dataset
    ...
    # --- 设置分层学习率 ---
    # 为预训练的骨干网络设置较低的学习率
    lr_pretrained_backbone = supcon_lr * 0.2  # 例如，预训练骨干的学习率是 supcon_lr 的十分之一
    ...
    # 这些模块在 EncoderNetwork.__init__ 中被初始化，所以它们的参数是可访问的
    # if hasattr(supcon_training_model.encoder, 'scat2a'):
    ...
    # 注意: nn.AdaptiveAvgPool2d 通常没有可学习参数，所以不需要加入优化器
    ...
    if pretrained_backbone_params: # 确保参数列表不为空
        optimizer_parameter_groups.append({
            'params': pretrained_backbone_params,
            'lr': lr_pretrained_backbone
        })
        print(f"Optimizer: Pretrained backbone parameters will use LR: {lr_pretrained_backbone}")

    if encoder_frontend_params: # 确保参数列表不为空
        optimizer_parameter_groups.append({
            'params': encoder_frontend_params,
            'lr': lr_new_parts 
        })
        print(f"Optimizer: Encoder frontend parameters will use LR: {lr_new_parts}")
    
    if projection_head_params: # 确保参数列表不为空
        optimizer_parameter_groups.append({
            'params': projection_head_params,
            'lr': lr_new_parts
        })
        print(f"Optimizer: Projection head parameters will use LR: {lr_new_parts}")

    if not optimizer_parameter_groups:
        raise ValueError("No parameters found to optimize. Check model structure and parameter grouping.")

    optimizer_supcon = torch.optim.AdamW(
        optimizer_parameter_groups, 
        # lr=supcon_lr, # 主学习率在此处不再需要，因为它已在组内定义
        weight_decay=1e-5 
    )
    print("Optimizer_supcon configured with layer-wise learning rates.")
...
# --- Phase 2: Prototypical Network Classification ---
    ...
    if len(prototype_compute_loader.dataset) > 0:
        prototypical_net.compute_prototypes(prototype_compute_loader, support_classes)
    else:
