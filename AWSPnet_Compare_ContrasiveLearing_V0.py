...
################################################################################
# 第一步：加载数据，并将标签转换为整数索引 (不再是One-Hot),监督对比学习，当前参数获得的损失较小
################################################################################
...
class EncoderNetwork(nn.Module):
    def __init__(self, 
        ...
        # self.scat = ScatLayer().to(device) # Original scat layer
    ...
    def forward(self, x):
        ...
        if out_features.ndim == 4: # Common for CNNs before flatten
            if out_features.shape[2] == 1 and out_features.shape[3] == 1: # Global Pooled features
                out_features = torch.flatten(out_features, 1)
            else: # Other 4D outputs, flatten starting from channel dim
                out_features = torch.flatten(out_features, 1) 
...
class SupConLoss(nn.Module):
    ...
    def forward(self, features: torch.Tensor, labels: Optional[torch.Tensor] = None, 
        ...
        """
        Args:
            features: Hidden_vector of shape [bsz, n_views, ...]. Assumed to be L2 normalized.
        ...
        logits_mask = torch.ones_like(mask) - torch.eye(batch_size * anchor_count, device=self.device).repeat(1, n_views // anchor_count if n_views > anchor_count else 1)[:,:batch_size*n_views] # Ensure correct shape for eye
        ...
        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True) + 1e-12) # Add small epsilon
...
if __name__ == "__main__":
    ...
    **MobileViT (2022)**: Combines lightweight CNNs with Transformers to create an efficient vision model for mobile devices, 
    achieving strong representational power while maintaining low computational complexity.
    ...
    **ShuffleNet (2018)**: Combines pointwise group convolution and channel shuffle, significantly reducing computational load while preserving accuracy.
    '''
    model_type = 'densenet'  # Example: 'resnet50', 'convnext_v2', 'mobilenet_v3', 'efficientnet_v2',  'shufflenet', 'densenet'
    ...
    supcon_epochs = 20 # E.g., 20-100 epochs, depends on dataset
    supcon_lr = 1e-4 # Tunable: 1e-3, 5e-4, 1e-4
    # projection_hidden_dim will be set after encoder init based on encoder.encoder_feature_dim
    projection_output_dim = 128 # Common for contrastive embeddings
