...
class DTCWT_Attention_Pretrained(nn.Module):
    ...
    def _register_hooks(self):
        ...
        target_layers = {
            'Z': self.scat2a,          # 第一层特征
            'att': self.attention_module,  # CBAM输出
            'reduced': self.channel_reducer[-1]  # 降维层最后输出
        }
        
        def get_hook(name):
            def hook(module, input, output):
                self.feature_maps[name] = output.detach()  # 分离计算图
            return hook
    ...
    def forward(self, x):
        ...
        # self.feature_maps = {}  # 清空上一轮特征
        x = x.to(self.device)
        Z = self.scat2a(x)
        
        # save Z to view, code here

        #
        # 应用CBAM注意力机制
        big_C = Z.shape[1]
        self.attention_module = CBAMBlock(channel=big_C, reduction=8, kernel_size=7).to(self.device)
        self.attention_module.register_forward_hook(lambda m, inp, out: self.feature_maps.update({"att": out.detach()}))

        att_features = self.attention_module(Z)
        # save att_features to view, code here
        ...
        self.channel_reducer.register_forward_hook(lambda m, inp, out: self.feature_maps.update({"reduced": out.detach()}))
        reduced_features = self.channel_reducer(att_features)
        # save reduced_features to view, code here
...
def extract_features_hook(model, dataloader, device):
    model.eval()
    model._register_hooks()  # 注册钩子
    ...
    with torch.no_grad():
        for batch_data, batch_labels in tqdm(dataloader):
            _ = model(batch_data.to(device))  # 前向传播（自动触发钩子）
            
            # 收集当前batch的特征（已通过钩子存储）
            for name, feat in model.feature_maps.items():
                # 全局平均池化处理卷积特征
                if len(feat.shape) == 4:  # [B,C,H,W]
                    feat = feat.mean(dim=[2, 3])  # 变为[B,C]
                all_features[name].append(feat.cpu().numpy())

            # 确保 labels 是类别索引（不是 one-hot）
            if batch_labels.dim() == 2:  # 如果是 one-hot
                batch_labels = torch.argmax(batch_labels, dim=1)
            all_labels.append(batch_labels.cpu().numpy())
            
    
    # 合并结果
    features_dict = {k: np.concatenate(v) for k, v in all_features.items()}
    labels = np.concatenate(all_labels)
    
    model.remove_hooks()  # 清理钩子
    return features_dict, labels

def visualize_tsne(features, labels, title="t-SNE Visualization", perplexity=30):
    """
    使用t-SNE降维并可视化特征
    参数:
        features: [N, D] 的numpy数组
        labels: [N,] 的类别标签
        title: 图像标题
        perplexity: t-SNE的核心参数，建议值5~50
    """
    # t-SNE降维
...
if __name__ == "__main__":
    ...
    # 1. 加载数据、标签
    test_dataset, val_dataset, num_classes = load_data_and_onehot_label(data_path="./Data/TestData_1.mat", label_path="./Data/Test_Label.mat", test_split=0.4)
    train_dataset, num_classes = Train_data_Gen(data_path="./Data/TestData_4.mat", label_path="./Data/Train_Label.mat")
    # 从测试集中划分验证集
    ...
    # 'shufflenet', 'mobilenet_v3', 'mobilevit',"resnet50"
    # model_type = 'convnext_v2'  # 可选的模型类型有: 'convnext_v2', 'efficientnet_v2', 'resnext',
    #  'shufflenet', 'mobilenet_v3', 'mobilevit', "resnet50"
    model_type = 'resnext'  # 可选的模型类型有: 'convnext_v2', 'efficientnet_v2', 'resnext', 
    ...
    # 提取特征（保持register_forward_hook的提取逻辑不变）
    features_dict, labels = extract_features_hook(trained_model, test_loader, device)
    ...
    for layer_name, features in features_dict.items():
        ...
        visualize_tsne(
            features, 
            labels,
            title=f"t-SNE of {layer_name} Features",
            perplexity=30  # 可调整
        )
    ...
    # 7. 加载保存的最佳模型进行预测
    torch.save(trained_model.state_dict(), f'saved_models/{model_type}_wavelet_model_best.pth')
    print("\nLoading best model for prediction...")

    trained_model.load_state_dict(torch.load(f'saved_models/{model_type}_wavelet_model_last.pth'))
